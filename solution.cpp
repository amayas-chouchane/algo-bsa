...
void Solution::fitness(const vector<double>& solution)
{
    switch (functionIndex)
    {
        case 1:  // Rosenbrock
            currentFitness = rosenbrock(solution);
            break;
        case 2:  // Rastrigin
            currentFitness = rastrigin(solution);
            break;
        case 3:  // Ackley
            currentFitness = ackley(solution);
            break;
        case 4:  // Schwefel
            currentFitness = schwefel(solution);
            break;
        default:
            currentFitness = numeric_limits<double>::infinity(); // Return high value for invalid function index
            break;
    }
}
